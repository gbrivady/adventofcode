class Solution:
    def part1(self, data):
        ...
        hands = [
            (self.label_to_number(hand[0]), int(hand[1]), self.get_score(hand[0]))
            for hand in hands
        ]
    ...
    def part2(self, data):
        ...
        hands = [
            (
                self.label_to_number(hand[0], wildcard=True),
                int(hand[1]),
                self.get_score(hand[0], wildcard=True),
            )
            for hand in hands
        ]
        hands = sorted(hands, key=lambda hand: (hand[2], hand[0]))
...
print(s.part1(open("input/07")))
print(s.part2(open("input/07")))
